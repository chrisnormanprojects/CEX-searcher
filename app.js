...
const els={over50Only:$('#over50Only'),query:$('#query'),maxPrice:$('#maxPrice'),superCategory:$('#superCategory'),category:$('#category'),onlineOnly:$('#onlineOnly'),hideZero:$('#hideZero'),searchBtn:$('#searchBtn'),locationBtn:$('#locationBtn'),sortBy:$('#sortBy'),results:$('#results'),message:$('#message'),status:$('#statusPill'),count:$('#resultCount'),cheapest:$('#cheapestPrice'),locationText:$('#locationText'),template:$('#cardTemplate')};
let products=[],dataMeta=null,loadedSelection='',loadSequence=0,controller=null,visibleLimit=100,lastGenerated=null,statusLabel='',loading=false;
...
function updateAge(){if(!lastGenerated||loading)return;const age=Date.now()-Date.parse(lastGenerated);setStatus(`${ageLabel(lastGenerated)}${age>7200000?' • OVERDUE':' • OK'}`,age<=7200000)}
...
function selection(){return `${els.superCategory.value}:${els.category.value}:${els.over50Only.checked}`}
function categoryCount(c){return els.over50Only.checked?(c.over50Listings||0):(c.under50Listings??c.cheapListings)}
function categoryFiles(c){return c.priceFiles?.[els.over50Only.checked?'over50':'under50']||c.files||[]}
function syncPriceControls(){els.maxPrice.disabled=els.over50Only.checked;document.querySelectorAll('[data-price]').forEach(b=>{b.disabled=els.over50Only.checked;b.classList.toggle('active',!els.over50Only.checked&&Number(b.dataset.price)===Number(els.maxPrice.value));});}
function populateDepartments(){els.superCategory.innerHTML='<option value="">Choose a department</option>';for(const s of [...dataMeta.superCategories].sort((a,b)=>a.name.localeCompare(b.name))){const o=document.createElement('option');o.value=String(s.id);o.textContent=`${s.name} (${dataMeta.categories.filter(c=>c.superCatId===s.id).reduce((n,c)=>n+categoryCount(c),0).toLocaleString()})`;els.superCategory.appendChild(o)}}
function populateCategories(){const id=Number(els.superCategory.value);els.category.innerHTML='<option value="">Choose a category</option><option value="all">All categories (larger download)</option>';const groups=new Map();for(const c of dataMeta.categories.filter(c=>c.superCatId===id)){const g=c.productLineName||'Other';if(!groups.has(g))groups.set(g,[]);groups.get(g).push(c)}for(const [g,cats] of [...groups].sort((a,b)=>a[0].localeCompare(b[0]))){const og=document.createElement('optgroup');og.label=g;for(const c of cats.sort((a,b)=>a.name.localeCompare(b.name))){const o=document.createElement('option');o.value=String(c.id);o.textContent=`${c.name} (${categoryCount(c).toLocaleString()})`;og.appendChild(o)}els.category.appendChild(og)}}
...
async function loadCatalog(){els.searchBtn.disabled=true;loading=true;setStatus('Loading…');try{const r=await fetch(`${DATA_URL}?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);dataMeta=await r.json();populateDepartments();populateCategories();products=[];lastGenerated=dataMeta.generatedAt;els.locationText.textContent=`${dataMeta.productCount.toLocaleString()} products indexed`;showMessage('Choose a department, then a category.');render()}catch(e){console.error(e);lastGenerated=null;setStatus('Unavailable');showMessage('The catalogue could not be loaded. Refresh to retry.')}finally{loading=false;els.searchBtn.disabled=!dataMeta;updateAge()}}
async function loadSelection(){
  ...
  if(els.over50Only.checked&&dataMeta.priceScope!=='all'){showMessage('Higher-priced items are not available yet. Refresh shortly.');return;}
  ...
  setStatus('Loading…');showMessage('Loading products…');
  try{
    let items=[];
    if(dataMeta.schemaVersion===2){
      const files=cats.flatMap(categoryFiles),parts=new Array(files.length);let cursor=0,done=0;
      ...
      items=parts.flat();
      if(items.length!==cats.reduce((n,c)=>n+categoryCount(c),0))throw new Error('Catalogue count mismatch');
    }else{
...
function catalogueInfo(){showMessage(`${dataMeta?.productCount.toLocaleString()} saved products ${dataMeta?.priceScope==='all'?'across all prices':'at £50 or less'}; ${dataMeta?.categoryCount} categories in ${dataMeta?.superCategoryCount} departments. ${dataMeta?.coverage==='complete'?'All search partitions passed coverage checks.':'Legacy catalogue: some categories may be incomplete.'} Scheduled hourly; runs can be delayed. Last refreshed ${new Date(dataMeta?.generatedAt).toLocaleString()}.`)}
...
els.superCategory.addEventListener('change',()=>{cancelLoad();populateCategories();lastGenerated=dataMeta.generatedAt;updateAge();els.locationText.textContent=`${dataMeta.productCount.toLocaleString()} products indexed`;showMessage('Choose a category, or select All categories to search the department.');});
els.category.addEventListener('change',loadSelection);
els.over50Only.addEventListener('change',()=>{const department=els.superCategory.value,category=els.category.value;cancelLoad();syncPriceControls();if(!dataMeta)return;populateDepartments();els.superCategory.value=department;populateCategories();els.category.value=category;if(department&&category)loadSelection();else showMessage('Choose a department and category to search.');});
